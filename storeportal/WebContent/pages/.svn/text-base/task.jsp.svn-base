...
<script type="text/javascript">
	...
	function doDeleteTask(val) {
		if(confirm('Are you sure?')){
			document.forms[0].action = 'TaskMain_detaildelete.do?portfolioId='+val;
			document.forms[0].submit();
		}else{
			return false;
		}
...
<jsp:include page="/include/header.jsp"/>  
   <div class="main">
   	<div class="headingBg" >
   		<div style="float:left;" class="heading" align="center"> <h1>Task Management</h1></div> 
	  	<div style="float:right; padding:2px; font-size:12px;  color:#e1e1de;">
			<s:url id="addurl" action="TaskMain_detailadd.do"/>
			<sj:dialog id="TaskDetail" href="%{addurl}" title="Portfolio: New"  autoOpen="false" modal="true" resizable="false" height="220" width="700"/>
	  		<sj:a href="#" indicator="indicator" button="true" openDialog="TaskDetail" buttonIcon="ui-icon-circle-plus">New Portfolio</sj:a>
	  	
	  		
	  	</div> 
   </div>
 
  <s:form action="TaskMain_search" method="post" theme="simple">
                			  ...
                			  <div class="boxCon">
			                   <display:table pagesize="20" cellspacing="1" cellpadding="5"  class="dataTable" id="data" name="groupList" requestURI="">
								...
								<display:column title="Action" headerClass="c" style="text-align:center;">
			                    	<s:url id="editurl" action="TaskMain_detailedit.do">
			                    		<s:param name="portfolioId" value="%{#attr.data.id}" />
			                    	</s:url>
									<sj:dialog id="TaskDetail_update_%{#attr.data.id}" href="%{editurl}" title="Task Portfolio : Edit"  autoOpen="false" modal="true" resizable="false"
										 height="230" width="700" />
									<sj:a href="#" indicator="indicator" openDialog="TaskDetail_update_%{#attr.data.id}">Edit</sj:a>|
									<sj:a href="#" indicator="indicator" onclick="doDeleteTask(%{#attr.data.id})">Del</sj:a>
			                    </display:column>
			                    	<s:url id="viewurl" action="TaskPostMain_taskpost.do">
			                    		<s:param name="postLevel" value="\'Y\'" ></s:param>
			                    		<s:param name="portfolioId" value="%{#attr.data.id}" ></s:param>
			                    	</s:url>
			                    	<s:property value="id"/>
			                    <display:column property="id" title="Portfolio ID" paramId="id" headerClass="c" style="text-align:center;" />
								<display:column property="mstName" title="Portfolio Name"    style="text-align:left;" />
